...
public class NrhpPropertyDatabase2 implements PropertyDatabase {
	/**
	 * 
	 */
	...
	public SearchResult search(String input) {
		...
		BinaryNrhpPropertyTree found = (BinaryNrhpPropertyTree) _tree.get(key);
		LinkedList<NrhpProperty> all =  found.traverse();
		...
		if (all != null) {
			for(NrhpProperty property : all) {
				if ((!toolong) || (toolong && property.matches(fullkey))) {
    ...
    private void processProperty(XMLStreamReader parser) {
    	...
    	String entry;
     	...
     	for (String id : _ids) {
     		BinaryNrhpPropertyTree properties = (BinaryNrhpPropertyTree) _tree.get(id);
     		if (properties == null) {
     			properties = new BinaryNrhpPropertyTree();
         		_tree.put(id, properties);
     		}
